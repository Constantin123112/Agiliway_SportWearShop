...
namespace SportsWearShop.Api.Domain.Identity.Services
{
    public class BasketService : IBasketService
    {
        ...
        public BasketService(IConfiguration configuration, ApiDbContext context)
        {
            _context = context;
        ...
        public async Task<string> Create(CreateBasket request)
        {
            var entity = new BasketEntity
            {
                ProductId = request.ProductId,
                Status_deleted = BasketDeleted.Active,
                UserId = GetUserId(request.UserLogin),
                CreatedAt = DateTime.Now
            };

            var resultEntityProductSizeId = await _context.Basket.AddAsync(entity);
            ...
            try
            {
                ...
                return $"Прдукт {productName} успішно додан в кошик !";
            }
            catch (Exception)
            {
        ...
        public List<ProductEntity> GetProducts(string login)
        {
            ...
            var products = from b in _context.Basket
                           join p in _context.Products on b.ProductId equals p.Id
                           join u in _context.Users on b.UserId equals u.Id
                           where u.Id == UserId && b.Status_deleted == BasketDeleted.Active
                           select new
                           {
                               Id = p.Id,
                               Name = p.Name,
        ...
        public bool Check(string login, long productId)
        {
            ...
            var products = from b in _context.Basket
                           join p in _context.Products on b.ProductId equals p.Id
                           join u in _context.Users on b.UserId equals u.Id
                           where u.Id == UserId && p.Id == productId && b.Status_deleted == BasketDeleted.Active
                           select p;
        ...
        public async Task<string> Delete(string login, long ProductId)
        {
            ...
            var t = from b in _context.Basket
                           join p in _context.Products on b.ProductId equals p.Id
                           join u in _context.Users on b.UserId equals u.Id
            ...
            foreach (BasketEntity item in t)
            {
                item.Status_deleted = BasketDeleted.Not_active;
            }
